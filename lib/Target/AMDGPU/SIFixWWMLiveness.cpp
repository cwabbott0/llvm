...
/// since writing %vgpr1 would only write to channels that would be clobbered
/// by the second write to %vgpr0 anyways. But if %vgpr1 is written with WWM
/// enabled, it would clobber even the inactive channels for which the
/// if-condition is false, for which %vgpr0 is supposed to be 0. This pass adds
/// an implicit use of %vgpr0 to the WWM instruction to make sure they aren't
